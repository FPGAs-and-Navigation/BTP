...
\begin{tikzpicture}[
  node distance=4mm,
  io/.style={draw, rounded corners, minimum height=1cm, minimum width=2.35cm, align=center, font=\tiny},
  ->, >=Stealth
]
\node[io] (data) {High-precision\\image--coordinate\\data};
\node[io, right=of data] (backbone) {Transformer\\vision backbone\\(replaces VGG16\textsuperscript{[1]})};
\node[io, right=of backbone] (multi) {+ IMU /\\magnetometer /\\gravimeter fusion};
\node[io, right=of multi] (out) {Latitude,\\Longitude};
\draw (data) -- (backbone);
\draw (backbone) -- (multi);
\draw (multi) -- (out);
\end{tikzpicture}
